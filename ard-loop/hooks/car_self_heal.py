...
def main():
    ...
    health = "GREEN" if not remaining else ("RED" if "fresh_harvest" in remaining or "supervision_live" in remaining else "YELLOW")
    ...
    log_heal("cycle_end", health=health, remaining=remaining, n_actions=len(actions_taken))

    # 회장/Alpha 다이제스트 — GREEN이고 아무 액션 없었으면 조용히(nagging 방지).
    if health != "GREEN" or actions_taken:
        print(f"[ARD 자가치유] 건강도: {health} | 초기 미충족: {initial_unmet or '없음'} | "
              f"조치 {len(actions_taken)}건 | 잔여: {remaining or '없음'}")
