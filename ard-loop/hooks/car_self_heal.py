...
DIGEST_RED_STREAK_N = 2  # 소화(digestion) RED가 N사이클 연속이면 GOAP 목표 충족이어도 escalate
                         #  (G2 정직화 2026-07-22 — '조용한 RED' 표면화. GOAP 목표는 확장 안 함)
...
def main():
    ...
    # ── 소화 정직화 (G2 2026-07-22) ────────────────────────────────────────────
    # digestion RED 연속 사이클을 추적 → GOAP 목표가 전부 충족(remaining=[])이어도
    # '조용한 RED'를 escalate로 표면화. 48h 스로틀로 나깅 방지. (GOAP 목표는 확장 안 함)
    prev = load(HEAL_STATE, {})
    digest_red_streak = (prev.get("digestion_red_streak", 0) + 1) if sup == "RED" else 0
    last_digest_esc_age = hours_since(prev.get("last_digest_escalate"))
    last_digest_escalate = prev.get("last_digest_escalate")
    escalated = False

    # (a) 복구 불가(미해결 목표 + RED) — 기존 경로.
    ...
    # (b) 소화 N사이클 연속 RED(AND 완화 = OR 경로) — GOAP 목표 충족이어도 발화.
    if (sup == "RED" and digest_red_streak >= DIGEST_RED_STREAK_N
            and (last_digest_esc_age is None or last_digest_esc_age >= SURFACE_STALE_H)):
        q = final_ws.get("queue", "?")
        msg = (f"🔴 [ARD 자가치유] 소화(digestion) {digest_red_streak}사이클 연속 RED — "
               f"큐 {q}건 내재화 정체. 기계(mech)는 도나 값을 못 냄 = self_heal이 못 고치는 영역.\n"
               f"CAR 소환 또는 Alpha 세션에서 큐 처리(car_pull.py) 필요.")
        push(msg)
        add_persistent_notice(msg, max_shows=5)
        log_heal("escalate_digestion_stall", streak=digest_red_streak, queue=q)
        last_digest_escalate = now_iso()
        escalated = True

    summary = {
        "ts": started,
        "health_after": health,          # 통합(더 심각한 쪽) = worst-of(mech, digestion)
        "health_mechanical": mech,       # GOAP 3목표(부품 가동)
        "health_digestion": sup,         # supervisor loop_health(값 산출)
        "digestion_red_streak": digest_red_streak,
        "last_digest_escalate": last_digest_escalate,
    ...
    log_heal("cycle_end", health=health, mech=mech, digestion=sup,
             digestion_red_streak=digest_red_streak,
             remaining=remaining, n_actions=len(actions_taken), escalated=escalated)
    ...
    if health != "GREEN" or actions_taken:
        print(f"[ARD 자가치유] 통합 {health} (기계 {mech}·소화 {sup or 'n/a'}"
              f"{f'·연속RED {digest_red_streak}' if sup=='RED' else ''}) | "
    ...
    # 최종 상태 = worst-of(mech, digestion)를 exit code로 표면화(schtask LastResult에 RED가 보이게).
