...
- **cadence(계속 보장):** `car_supervisor.py`가 마지막 조직 리뷰로부터 **21일** 경과 시 `~/.claude/ard/org_state.json`에 `due=true` → `car_dispatch.py`가 SessionStart에 "CAR 소환해 조직 리뷰"를 surface(**도구 큐와 독립**). 새 상주 프로세스 없음 — 기존 주기 실행 재사용.
