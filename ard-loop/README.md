...
| 경로 | 역할 | 실행 위치 |
|------|------|----------|
| `config/sources.json` | 유튜브 채널 + 깃허브 레포 + 신뢰출처 allowlist | → `~/.claude/ard/config/` |
| `harvester/car_harvester.py` | HARVEST: RSS→자막→큐 | Task Scheduler 일 1회 |
| `hooks/car_dispatch.py` | UNDERSTAND 트리거(큐 surface) | SessionStart |
| `hooks/car_supervisor.py` | 감독·평가·자가복구 | 스케줄/주기 |
| `templates/insight_template.md` | CAR 인사이트 구조 | CAR이 채움 |
| `install/install.ps1` | 의존성·복사·스케줄 등록 | 회장 1회 |
