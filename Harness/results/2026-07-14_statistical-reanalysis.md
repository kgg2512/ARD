# 하네스 통계 재분석 — 무엇을 신뢰할 수 있나 (2026-07-14)

> `harness_stats.py`(신설) MODE1 재분석. 회장 지시: "N-반복 유의성 → 헌법개정 결정의 데이터 신뢰성 확보."
> 핵심: 현 데이터는 스킬당 **생성 N=1**. judge 일치도 × |Δ|로 신뢰등급을 매겨 '믿을 결론'과 '노이즈'를 분리한다.
> 방법: `python harness_stats.py --reanalyze data/2026-07-01_panel_raw.json` (통계 정확성 `--selftest` 9/9 PASS)
...
- 3차 실험(`experiments/2026-07-01_exp3-*`)은 **variant당 N≥3 생성**으로 설계 → `harness_stats.py` MODE2 `nrep_stats()`가 평균·표준편차·**95% 부트스트랩 CI·Cohen's d·유의판정** 산출 → 진짜 유의성. 그때 MEDIUM 6개가 유의로 굳는지 노이즈로 무너지는지 판가름 난다.
- 이 모듈은 **point estimate 단독 산출을 거부**한다(N=1이면 error, N≥2면 항상 분산+CI 동반). 다른 Claude·회장 지적("노이즈를 신호로 착각")의 코드 차단.

## 소유
ARD Harness / CAR. 관련: `PROJECT.md`, `harness_stats.py`, [[project-ard-harness]]
