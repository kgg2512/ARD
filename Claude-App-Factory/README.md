...
캐러셀 8장 = 표지(1) + 도구 #1~#6(슬라이드 2~7) + CTA(8). **실제 도구는 6개, 전부 판독 완료** [확인].

| # | 도구 | 공정 | 무엇 | 왜 | 무료 한도 | 회장 보유 여부 |
|---|------|------|------|-----|----------|---------------|
| #1 | **Claude Code** | 두뇌 | AI 코딩 에이전트. 자연어→코드, 도구 제어 | 시스템 전체의 중심. 나머지는 전부 Claude가 조작하는 손발. **Rule #1: 아키텍처·의존성을 이해하라, 전부 바이브 코딩 하지 마라, CTO와 보안·기술 감사하라** | Pro/Max 구독 | ✅ 보유·사용 중 |
...
| #3 | **Dribbble** | 설계 | UI/UX 디자인 영감 갤러리 | 화면·카드·색상·레이아웃 참고. "남들은 어떻게 디자인했나" | 무료(열람) | 🟡 **6개 중 유일하게 MCP 없음.** 순수 수동(사람 눈) 노드. G2 자동화 디자인 = Figma·Stitch MCP, Dribbble은 선택적 참고만 |
...
> **Rule #1의 의미 (회장 정합):** leadgenman은 "솔로여도 CTO와 감사하라"고 했다. 회장은 이걸 이미 **AI CTO 에이전트(g2-cto) + Spec Kit(바이브 코딩 차단) + CISO/CSO 검증 게이트**로 구현해 뒀다 — 사진보다 한 단계 앞서 있다.

> **결론:** 회장은 이 스택의 핵심을 **이미 80% 보유**하고 있다(MCP로 Vercel·Supabase·GitHub·Figma·Stitch 연결됨). 실제로 새로 필요한 건 ①Conductor의 Windows 대안 ②PostHog뿐. Dribbble은 도구가 아니라 습관(영감 열람). 자세한 매핑은 [STACK_MAPPING.md](./STACK_MAPPING.md).
...
- **[확인]** 사진 도구 #1~#6 6장 전부 직접 판독함. 도구명·역할·"Mac app"·Rule #1·무료성 일부는 사진 본문에 명시됨. 도구는 6개로 확정(8장 = 표지+#1~6+CTA).
- **[확인]** Dribbble은 6개 중 유일하게 MCP/API 연동이 없는 수동 노드 — 회장 지적 반영.
- **[추정]** 무료 한도 수치(PostHog 100만/Supabase 2개 등)는 파라미터 지식 기반이라 도입 직전 공식 페이지 재확인 필요.
